			...
			try
			{
				...
				for (int i = 0; i < pingCount; i++)
				{
					...
					try
					{
                        ...
                        socket.SendTimeout = pingTimeout;
						...
						else
						{
							...
							if (byteCount == SOCKET_ERROR||receiveBuffer[20]!=0)
							{
								result = PingResponseType.ConnectionError;
								response.ResponseTimes[i] = Constants.InvalidInt;
							}
							...
							{
								//Record time
								response.ResponseTimes[i] = (int)_stopwatch.ElapsedMilliseconds;
